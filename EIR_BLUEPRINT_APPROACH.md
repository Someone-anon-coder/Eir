...
2. **In-page capture script:** a single self-contained function passed to `page.evaluate` against the resolved element handle; returns plain JSON; Tailwind-noise class filtering; nothing user-data-ish (P7). Fire-and-forget — a capture failure logs and never fails a test. The browser round-trip starts concurrently with the action (not strictly after it resolves) and is only *recorded* on success — see BLUEPRINT.md §7.2 for why (navigational actions destroy their own element before a post-success capture could reach it).
